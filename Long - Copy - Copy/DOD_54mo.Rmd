...
```
8 verb errors and 3 Admin Errors. Some of those verb errors are also classified as admin errors. This is because the audio cut out on a number a few Zoom sessions and the child didn't hear the target verb and 
...
```{r}
DOD_54mo <- filter(DOD_54mo, !is.na(RESPONSE.CODE) & Drop==0) # 1013
...
DOD_54mo %>% 
  ...
  count()
```
Again, it looks like the pattern is different on the levels of aggregate data and indiviaul verbs. 
...
m3b <- DOD_54mo3 %>%
  ...
  filter(k < .7) %>%
 brm(RESPONSE.CODE ~ 1 +Prime.Type*biasmatch + (1 +Prime.Type*biasmatch  | Blind_ID) + (1 +Prime.Type*biasmatch | Target.verb), logit_prior, family="bernoulli", iter=4000, cores=4, file="54mo/output/m3b.rds", data=.)
```
...
m4b <- DOD_54mo4 %>%
  ...
  filter(k < .7) %>%
 brm(RESPONSE.CODE ~ 1 +Prime.Type*biasmatch + (1 +Prime.Type*biasmatch  | Blind_ID) + (1 +Prime.Type*biasmatch | Target.verb), logit_prior, family="bernoulli", iter=4000, cores=4, file="54mo/output/m4b.rds", data=.)
```

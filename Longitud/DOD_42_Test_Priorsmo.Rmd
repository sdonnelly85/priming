...
```
15 verb errors, 7 admin errors 1 disallowed prompt = 23.
...
## All Participants
Set somewhat informative prior given 
```{r}
...
m3b <- DOD_42mo3 %>%
  ...
  filter(k < .7) %>%
 brm(RESPONSE.CODE ~ 1 +Prime.Type*biasmatch + (1 +Prime.Type*biasmatch  | Blind_ID) + (1 +Prime.Type*biasmatch | Target.verb), logit_prior, family="bernoulli", iter=4000, cores=4, file="42mo/output/m3b.rds", data=.)
```
...
hypothesis(m3b, "Prime.Type:biasmatchmismatch > 0") # Bias mismatch
```
...
m4b <- DOD_42mo4 %>%
  ...
  filter(k < .7) %>%
 brm(RESPONSE.CODE ~ 1 +Prime.Type*biasmatch + (1 +Prime.Type*biasmatch  | Blind_ID) + (1 +Prime.Type*biasmatch | Target.verb), logit_prior, family="bernoulli", iter=4000, cores=4, file="42mo/output/m4b.rds", data=.)
```
